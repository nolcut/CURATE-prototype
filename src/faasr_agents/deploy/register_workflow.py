...
def verify_containers(workflow_data):
    ...
    action_containers = workflow_data.get("ActionContainers", {})
    if not action_containers:
        return

    # The authoritative whitelist ships inside this package, so it is always
    # present. A CWD copy (scripts/native_containers.txt) may EXTEND it, but the
    # check must never be silently skipped just because a file is absent —
    # the only way to allow custom containers is CUSTOM_CONTAINER=true above.
    packaged = os.path.join(os.path.dirname(__file__), "native_containers.txt")
    if not os.path.exists(packaged):
        logger.error(
            f"Native-container whitelist missing from package: {packaged}. "
            "Cannot validate containers; set CUSTOM_CONTAINER=true only if you "
            "intend to use custom containers."
        )
        sys.exit(1)

    native_containers: set[str] = set()
    for path in (packaged, os.path.join("scripts", "native_containers.txt")):
        if os.path.exists(path):
            with open(path, "r") as f:
                native_containers.update(line.strip() for line in f if line.strip())

    for container in action_containers.values():
...
def deploy_to_github(workflow_data):
    ...
    github_token = (os.getenv("GH_PAT") or "").strip()
    ...
    # GITHUB_REPOSITORY is only set inside the Actions runner; when running
    # locally fall back to the configured action repo (same vars the upload
    # step uses) so deployment targets the right repo.
    repo_name = os.getenv("GITHUB_REPOSITORY")
    if not repo_name:
        gh_user = os.getenv("FAASR_GH_USERNAME")
        action_repo = os.getenv("FAASR_ACTION_REPO")
        if gh_user and action_repo:
            repo_name = f"{gh_user}/{action_repo}"

    if not repo_name:
        logger.error(
            "Target repository not set — provide GITHUB_REPOSITORY, or "
            "FAASR_GH_USERNAME and FAASR_ACTION_REPO."
        )
        sys.exit(1)
    ...
    try:
        ...
        logger.error(f"Error deploying to GitHub ({type(e).__name__}): {detail}")
        if hasattr(e, "data"):
            logger.error(f"Error details: {e.data}")
        if hasattr(e, "status"):
            logger.error(f"HTTP status: {e.status}")
...
def main(workflow_data: dict | None = None, workflow_file: str | None = None):
    """
    Deploy a workflow. Can be called programmatically with workflow_data dict,
    or from CLI with workflow_file path.
    """
