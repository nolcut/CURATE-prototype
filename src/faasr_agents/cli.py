...
Flags:
    ...
    --opencode          # use OpenCode for function coding/testing
...
def main() -> None:
    ...
    # --anthropic-api selects the planning/review LLM provider. --opencode
    # independently selects the function-coding backend. Without either flag,
    # the existing Bedrock + Claude Code SDK behavior remains unchanged.
    ...
    args = [a for a in args if a not in (
        "--opus", "--sonnet", "--anthropic-api", "--opencode", "--debug",
    )]
    set_model_tier(tier)
    if anthropic_api:
