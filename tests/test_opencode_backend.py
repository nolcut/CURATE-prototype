...
class BackendSelectionTests(unittest.TestCase):
    def test_claude_code_remains_default_for_existing_providers(self):
        with (
            patch.object(llm, "selected_fga_backend", "claude-code"),
        ...
        with (
            patch.object(llm, "selected_fga_backend", "claude-code"),
            patch.object(llm, "selected_provider", "anthropic"),
        ):
            self.assertEqual(llm.get_fga_backend(), "claude-code")
...
class DependencyCleaningTests(unittest.TestCase):
    def test_non_package_empty_markers_are_discarded(self):
        self.assertEqual(
            clean_dependencies(
                ["no dependencies", "None", "N/A", "stdlib-only", "matplotlib"]
            ),
            ["matplotlib"],
        )
...
class OpenCodeConfigurationTests(unittest.TestCase):
    ...
    def test_subprocess_environment_hides_deployment_secrets(self):
        ...
        self.assertEqual(env["NVIDIA_API_KEY"], "model-secret")
