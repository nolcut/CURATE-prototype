...
- **LLM credentials.** `BEDROCK_API_KEY` and `AWS_REGION` for AWS Bedrock, the default
  ...
  `FAASR_OPENCODE_MODEL` optionally selects the model used by OpenCode during
  function generation, using OpenCode's `provider/model` format. For an
  `ollama/...` model, `FAASR_OLLAMA_BASE_URL` selects the Ollama endpoint and
...
NVIDIA NIM, or another OpenAI-compatible endpoint. Configure that provider in
OpenCode, then put the exact model name reported by `opencode models` in
...
```sh
# OpenAI for planning/review; OpenCode's configured model for function coding
uv run faasr-agents --openai-api --opencode
